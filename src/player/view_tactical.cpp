...
View_Tactical::doDefault( PlayerAgent * agent )
{
    ...
    const double ball_dist = self_next.dist( ball_pos );
    const double next_ball_dist = self_next.dist( wm.ball().pos() + wm.ball().vel() );
    ...
    if ( ball_dist > 40.0
         && next_ball_dist > ServerParam::i().tackleDist() )
    {
        dlog.addText( Logger::TEAM,
