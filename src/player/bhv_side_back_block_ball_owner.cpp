 ...
 */
bool
Bhv_SideBackBlockBallOwner::execute( PlayerAgent * agent )
{
    dlog.addText( Logger::ROLE,
 ...
 */
bool
Bhv_SideBackBlockBallOwner::doIntercept( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
 ...
 */
bool
Bhv_SideBackBlockBallOwner::doBlockMove2013( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    ...
    //
    // turn dasn or omni dash simulation
    //
    MoveSimulator::Result result_move;
    Vector2D result_point = simulateTurnDashOrOmniDash2013( wm, opponent_ball_pos, center_pos, &result_move );

    //
    // dash only simulation
    //
    if ( result_point.isValid()
         && result_move.turn_step_ > 0 )
    {
        MoveSimulator::Result dash_only_move;
        Vector2D dash_only_result_point = simulateDashOnly2013( wm, opponent_ball_pos, center_pos, result_point,
                                                                &dash_only_move );
        if ( dash_only_result_point.isValid() )
        {
            result_point = dash_only_result_point;
            result_move = dash_only_move;
            dlog.addText( Logger::ROLE,
                          __FILE__":(doBlockMove2013) found dash only" );
        }
    }

    //
    // perform turn or dash
    //

    if ( result_point.isValid()
         && result_move.turn_step_ >= 0
         && result_move.dash_step_ >= 0 )
    {
        //const double tolerance = std::max( 0.5, opponent_ball_pos.dist( result_point ) * 0.08 );
        const double tolerance = std::max( 0.5, wm.ball().pos().dist( result_point ) * 0.08 );

        dlog.addText( Logger::ROLE,
                      __FILE__":(doBlockMove2013) found pos=(%.2f %.2f) turn=%d dash=%d",
                      result_point.x, result_point.y,
                      result_move.turn_step_, result_move.dash_step_ );
        dlog.addRect( Logger::ROLE,
                      result_point.x - 0.3, result_point.y - 0.3, 0.6, 0.6, "#0F0" );
        agent->debugClient().setTarget( result_point );
        agent->debugClient().addCircle( result_point, tolerance );
        agent->debugClient().addLine( opponent_ball_pos, center_pos );
        agent->debugClient().addRectangle( M_bounding_rect );

        const Vector2D self_next = wm.self().pos() + wm.self().vel();
        if ( self_next.dist2( result_point ) > std::pow( tolerance, 2 ) )
        {
            if ( result_move.turn_step_ > 0 )
            {
                agent->debugClient().addMessage( "BlockTurn%.0f", result_move.turn_moment_ );
                agent->doTurn( result_move.turn_moment_ );
                return true;
            }

            if ( result_move.dash_step_ > 0 )
            {
                agent->debugClient().addMessage( "BlockDash%.0f:%.0f",
                                                 result_move.dash_power_, result_move.dash_dir_ );
                agent->doDash( result_move.dash_power_, result_move.dash_dir_ );
                return true;
            }
        }
        ...
        // already there
        //
        Vector2D turn_target_point = result_point;
        Vector2D ball_next = wm.ball().pos() + wm.ball().vel();
        if ( ball_next.dist( self_next ) > 5.0 )
        {
            turn_target_point = ball_next;
            dlog.addText( Logger::ROLE,
                          __FILE__":(doBlockMove2013) turn to ball" );
            agent->debugClient().addMessage( "BlockTurnToBall" );
        }
        else if ( opponent->bodyCount() <= 1 )
        {
            turn_target_point = opponent->inertiaFinalPoint()
                + Vector2D::from_polar( 30.0, opponent->body() );
            dlog.addText( Logger::ROLE,
                          __FILE__":(doBlockMove2013) turn to opponent body line" );
            agent->debugClient().addMessage( "BlockTurnToBodyLine" );
        }
        else
        {
            turn_target_point = result_point;

            dlog.addText( Logger::ROLE,
                          __FILE__":(doBlockMove2013) turn to move target point" );
            agent->debugClient().addMessage( "BlockTurnToPoint" );
        }

        dlog.addRect( Logger::ROLE,
                      turn_target_point.x - 0.2, turn_target_point.y - 0.2, 0.4, 0.4, "#00F" );
        dlog.addLine( Logger::ROLE,
                      wm.self().pos(), turn_target_point, "#00F" );

        Body_TurnToPoint( turn_target_point ).execute( agent );
        return true;
    }

    dlog.addText( Logger::ROLE,
                  __FILE__":(doBlockMove2013) false: not found" );
 ...
 */
Vector2D
Bhv_SideBackBlockBallOwner::simulateTurnDashOrOmniDash2013( const WorldModel & wm,
                                                            const Vector2D & opponent_ball_pos,
 ...
 */
Vector2D
Bhv_SideBackBlockBallOwner::simulateDashOnly2013( const rcsc::WorldModel & wm,
 ...
 */
bool
Bhv_SideBackBlockBallOwner::doBlockMove2012( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    ...
    const Line2D block_line( opponent_ball_pos, center_pos );
    const Segment2D block_segment( opponent_ball_pos, center_pos );
    ...
    Param param;
    simulate2012( wm, opponent_ball_pos, center_pos,
    ...
    if ( param.cycle_ <= 0 )
    {
        target_point = opponent_ball_pos * 0.3 + param.point_ * 0.7;
        dlog.addText( Logger::ROLE,
    ...
    if ( wm.self().pos().dist( target_point ) < dist_thr + 0.2 )
    {
        dist_thr = 0.4;
        dlog.addText( Logger::ROLE,
                      __FILE__":(execute) change tolerance to %.1f", dist_thr );
    }
    ...
    // block, but wait
    //
    if ( doWait( agent, param, block_segment ) )
    {
        return true;
    }
                         ...
                         ).execute( agent ) )
    {
        ...
        agent->debugClient().addMessage( "SB:Block:Go1" );
        dlog.addText( Logger::ROLE,
                      __FILE__": go to point(1) (%.1f %.1f) thr=%.3f cycle=%d stamina=%.1f",
                      target_point.x, target_point.y,
                      dist_thr,
                      param.cycle_,
                      param.stamina_ );
    }
    else if ( Body_GoToPoint( target_point,
                              ...
                              ).execute( agent ) )
    {
        agent->debugClient().setTarget( target_point );
        agent->debugClient().addCircle( target_point, dist_thr );
        agent->debugClient().addMessage( "SB:Block:Go2" );
        dlog.addText( Logger::ROLE,
    ...
    else
    {
        ...
        if ( ( body_angle - wm.self().body() ).abs() < 1.0 )
        {
            ...
            agent->debugClient().setTarget( target_point );
            agent->debugClient().addCircle( target_point, dist_thr );
            agent->debugClient().addMessage( "SB:Block:SideDash%.0f:%.0f",
...
#endif
        {
            ...
            agent->debugClient().setTarget( target_point );
            agent->debugClient().addCircle( target_point, dist_thr );
            agent->debugClient().addMessage( "SB:Block:TurnTo%.0f",
                                             body_angle.degree() );
            dlog.addText( Logger::ROLE,
 ...
 */
bool
Bhv_SideBackBlockBallOwner::doWait( PlayerAgent * agent,
                                    const Param & param,
 ...
 */
void
Bhv_SideBackBlockBallOwner::simulate2012( const WorldModel & wm,
                                          const Vector2D & opponent_ball_pos,
                                          const Vector2D & center_pos,
                                          ...
                                          Param * param )
{
    ...
    const int opponent_penalty_step = ( on_block_line
                                        ? 3
                                        : opponent_step == 0
                                        ? 0
                                        : 1 );
    ...
    for ( double len = min_length;
          ...
          len += dist_step, ++count )
    {
        Vector2D target_point = opponent_ball_pos + unit_vec * len;
        ...
        double stamina = 0.0;
        int n_turn = 0;
        int self_step = predictSelfReachCycle( wm,
                                               target_point,
                                               dist_thr,
                                               save_recovery,
                                               &n_turn,
        ...
        double opp_dist = std::max( 0.0,
                                    opponent_ball_pos.dist( target_point ) - opponent_type->kickableArea() * 0.8 );
        int opponent_reach_step
            = opponent_step
            + opponent_penalty_step
            + opponent_type->cyclesToReachDistance( opp_dist );
#ifdef DEBUG_PRINT
        debug_paint_target( count, target_point );
        dlog.addText( Logger::ROLE,
                      "__ pos=(%.1f %.1f) opp_dist=%.3f opp_cycle=%d(%d:%d) self_step=%d(t:%d) stamina=%.1f",
                      target_point.x, target_point.y,
                      opp_dist,
                      opponent_reach_step, opponent_step, opponent_penalty_step,
                      self_step, n_turn,
                      stamina );
#endif
        if ( self_step <= opponent_reach_step - 2
             || ( self_step <= 3 && self_step <= opponent_reach_step - 1 )
             || ( self_step <= 2 && self_step <= opponent_reach_step )
             //         if ( self_step <= opp_cycle - 2
             //              || ( self_step <= 1 && self_step < opp_cycle )
             //              || ( opp_cycle >= 8 && self_step <= opp_cycle - 1 )
             //              || ( opp_cycle >= 12 && self_step <= opp_cycle )
             //              || ( opp_cycle >= 16 && self_step <= opp_cycle + 1 )
             )
        {
            best.point_ = target_point;
            best.turn_ = n_turn;
            best.cycle_ = self_step;
    ...
    if ( ! on_block_line
         && best.turn_ > 0 )
    {
        simulateDashes( wm, opponent_ball_pos, center_pos,
    ...
    if ( best.point_.isValid() )
    {
        *param = best;
        dlog.addText( Logger::ROLE,
                      __FILE__": (simulate) target_point=(%.2f %.2f) reach_cycle=%d",
 ...
 */
void
Bhv_SideBackBlockBallOwner::simulateDashes( const WorldModel & wm,
                                            const Vector2D & opponent_ball_pos,
                                            const Vector2D & center_pos,
                                            const Rect2D & bounding_rect,
                                            const double & dist_thr,
 ...
 */
int
Bhv_SideBackBlockBallOwner::predictSelfReachCycle( const WorldModel & wm,
