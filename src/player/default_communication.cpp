 ...
 */
void
DefaultCommunication::updateCurrentSender( const PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
 ...
 */
bool
DefaultCommunication::shouldSayBall( const PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
         ...
         && wm.ball().seenVelCount() == 0 )
    {
        ...
        dlog.addText( Logger::COMMUNICATION,
                      "__ prev_vel=(%.2f %.2f) r=%.3f",
                      wm.prevBall().vel().x, wm.prevBall().vel().y,
                      prev_ball_speed );
        dlog.addText( Logger::COMMUNICATION,
                      "__ cur_vel=(%.2f %.2f) r=%.3f",
    ...
    if ( wm.self().isKickable() )
    {
        if ( wm.lastKickerSide() != wm.ourSide()
             || wm.lastKickerUnum() != wm.self().unum() )
        {
            dlog.addText( Logger::COMMUNICATION,
                          __FILE__": (shouldSayBall) kicker changed to me" );
            return true;
        }
 ...
 */
bool
DefaultCommunication::shouldSayOpponentGoalie( const PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
 ...
 */
bool
DefaultCommunication::goalieSaySituation( const rcsc::PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
 ...
 */
bool
DefaultCommunication::sayBallAndPlayers( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    ...
    if ( wm.self().isKickable() )
    {
        ...
        else // if ( agent->effector().queuedNextBallKickable() )
        {
    ...
    {
        objects[0].score_ = -100.0;

        if ( ! wm.opponentsFromBall().empty() )
        {
            const PlayerObject * opponent = wm.opponentsFromBall().front();
            if ( opponent->distFromBall()
                 < opponent->playerTypePtr()->kickableArea() + opponent->distFromBall() * 0.5 )
            {
                objects[0].score_ = -65535.0;
            }
        }
 ...
 */
bool
DefaultCommunication::sayStamina( PlayerAgent * agent )
{
    const int current_len = agent->effector().getSayMessageLength();
    if ( current_len == 0 )
    {
    ...
    const int available_len = ServerParam::i().playerSayMsgSize() - current_len;
    if ( available_len < StaminaCapacityMessage::slength() )
    {
        return false;
    }

    agent->addSayMessage( new StaminaCapacityMessage( agent->world().self().staminaCapacity() ) );

    dlog.addText( Logger::COMMUNICATION,
                  __FILE__": say self stamina capacity" );
    return true;
}
 ...
 */
bool
DefaultCommunication::sayRecovery( PlayerAgent * agent )
{
    const int current_len = agent->effector().getSayMessageLength();
    const int available_len = ServerParam::i().playerSayMsgSize() - current_len;
...
DefaultCommunication::attentiontoSomeone( PlayerAgent * agent )
{
    ...
    if ( wm.gameMode().type() != GameMode::PlayOn )
    {
        ...
        if ( kicker
             && kicker->unum() != Unum_Unknown
             && kicker->distFromSelf() < 20.0
             && kicker->distFromBall() < kicker->playerTypePtr()->kickableArea() )
        {
            agent->doAttentionto( wm.ourSide(), kicker->unum() );
            agent->debugClient().addMessage( "AttKicker%d", kicker->unum() );
            return;
        }
         ...
         && wm.interceptTable()->selfReachCycle() <= 3 )
    {
             ...
             && currentSenderUnum() != Unum_Unknown )
        {
            ...
            agent->debugClient().addMessage( "AttCurSender(1)%d", currentSenderUnum() );
            dlog.addText( Logger::COMMUNICATION,
                          __FILE__": attentionto current sender(1) unum=%d", currentSenderUnum() );
        }
        else
        {
            std::vector< const PlayerObject * > candidates;
    ...
    if ( fastest_teammate
         ...
         && mate_min <= 2 + std::min( 4, fastest_teammate->posCount() )
    ...
    {
        agent->doAttentionto( wm.ourSide(), currentSenderUnum() );
        agent->debugClient().addMessage( "AttCurSender(2)%d", currentSenderUnum() );
        dlog.addText( Logger::COMMUNICATION,
                      __FILE__": attentionto current sender(2) unum=%d", currentSenderUnum() );
    }
    else
    {
