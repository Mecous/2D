...
#include "bhv_mark_go_to_point.h"
 ...
 */
bool
Bhv_MarkGoToPoint::execute( PlayerAgent * agent )
{
#ifdef DEBUG_PRINT
    dlog.addText( Logger::ACTION,
                  __FILE__": (execute) target=(%.2f %.2f) dist_thr=%.3f",
    ...
    const Vector2D inertia_point = wm.self().inertiaPoint( MOVE_STEP );

    //
    // already there
    //
    if ( inertia_point.dist2( M_target_point ) < std::pow( M_dist_thr, 2 ) )
    {
#ifdef DEBUG_PRINT
        dlog.addText( Logger::ACTION,
 ...
 */
void
Bhv_MarkGoToPoint::checkGoalPost( const PlayerAgent * agent )
{
 ...
 */
bool
Bhv_MarkGoToPoint::doOmniDash( PlayerAgent * agent )
{
    const ServerParam & SP = ServerParam::i();
    const WorldModel & wm = agent->world();
    ...
    if ( target_rel.r2() > std::pow( M_dist_thr + 0.2, 2 ) )
    {
#ifdef DEBUG_PRINT
        dlog.addText( Logger::ACTION,
    ...
    const double max_dash_power = SP.maxDashPower();
          ...
          dir += dash_angle_step )
    {
        ...
        const int max_cycle = std::min( 3, MOVE_STEP );
        ...
        my_pos = wm.self().playerType().inertiaPoint( my_pos, rel_vel, MOVE_STEP );
        ...
        for ( ; cycle < max_cycle; ++cycle )
        {
            ...
            double dash_power = std::min( available_stamina, max_dash_power );
 ...
 */
bool
Bhv_MarkGoToPoint::doGoToTurn( PlayerAgent * agent )
{
    const ServerParam & SP = ServerParam::i();
    const WorldModel & wm = agent->world();

    const Vector2D inertia_pos = wm.self().inertiaPoint( MOVE_STEP );
    ...
    if ( turn_moment.abs() > max_turn
         && turn_moment.abs() > 150.0
         && target_dist < M_dist_thr + 0.3
...
// #endif
    ...
    turn_thr = std::max( 15.0, turn_thr );
 ...
 */
bool
Bhv_MarkGoToPoint::doDash( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    ...
    dash_power = std::min( dash_power, ServerParam::i().maxDashPower() );
    if ( M_back_mode )
    {
 ...
 */
void
Bhv_MarkGoToPoint::doMarkTurn( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
 ...
 */
void
Bhv_MarkGoToPoint::setTurnNeck( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    ...
    {
        dlog.addText( Logger::MARK,
                      __FILE__": (doTurnNeck) mark target is null." );
        agent->debugClient().addMessage( "Mark:Neck:Null" );
        agent->setNeckAction( new Neck_TurnToBallOrScan( 0 ) );
        return;
    }

    if ( M_mark_target->posCount() > 0 )
    {
        dlog.addText( Logger::MARK,
                      __FILE__": (doTurnNeck) check mark target, posCount>0" );
        agent->debugClient().addMessage( "Mark:Neck:Target1" );

        Vector2D pos = M_mark_target->pos();
        pos += M_mark_target->vel() * 2.0;

        agent->setNeckAction( new Neck_TurnToPoint( pos ) );
        return;
    }
    ...
    {
        dlog.addText( Logger::MARK,
                      __FILE__": (doTurnNeck) check ball force" );
        agent->debugClient().addMessage( "Mark:Neck:BallForce" );
              ...
              && wm.ball().distFromSelf() < 10.0 ) )
    {
        ...
        agent->debugClient().addMessage( "Mark:Neck:Ball" );

        agent->setNeckAction( new Neck_TurnToBallOrScan( 0 ) );
        return;
    }

    if ( M_mark_target->ghostCount() > 1 )
    {
        dlog.addText( Logger::MARK,
                      __FILE__": (doTurnNeck) check mark target, ghostCount>=2" );
        agent->debugClient().addMessage( "Mark:Neck:Ghost2" );

        agent->setNeckAction( new Neck_ScanField() );
        return;
    }

    if ( M_mark_target->ghostCount() > 0 )
    {
        dlog.addText( Logger::MARK,
                      __FILE__": (doTurnNeck) check mark target, ghostCount>0" );
        agent->debugClient().addMessage( "Mark:Neck:Ghost1" );

        agent->setNeckAction( new Neck_TurnToPoint( M_mark_target->pos() ) );
        return;
    }

    dlog.addText( Logger::MARK,
                  __FILE__": (doTurnNeck) check mark target, normal" );
    agent->debugClient().addMessage( "Mark:Neck:Mark" );

    agent->setNeckAction( new Neck_TurnToPlayerOrScan( M_mark_target, 0 ) );
}
