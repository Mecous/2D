...
/*!
  \file bhv_precise_shoot.cpp
  \brief precisely calculated shoot behavior using ShootGenerator
*/
...
void
paint_shoot_course( const Vector2D & first_ball_pos,
                    const Vector2D & first_ball_vel )
{
    ...
    for ( int i = 0; i < 30; ++i )
    {
        ...
        dlog.addRect( Logger::SHOOT,
                      ball_pos.x - 0.05, ball_pos.y - 0.05,
                      0.1, 0.1,
                      "#00ff00" );

        if ( ball_pos.x > ServerParam::i().pitchHalfLength() + 0.5 )
        {
            break;
        }
    }
 ...
 */
bool
Bhv_Shoot::execute( PlayerAgent * agent )
{
 ...
 */
bool
Bhv_Shoot::doHoldTurn( PlayerAgent * agent,
