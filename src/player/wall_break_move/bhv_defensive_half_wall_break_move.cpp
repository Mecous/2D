...
#include "bhv_defensive_half_wall_break_move.h"
 ...
 */
class IntentionDefensiveHalfWallBreakMove
...
public:
    ...
    IntentionDefensiveHalfWallBreakMove( const Vector2D & target_point,
 ...
 */
bool
IntentionDefensiveHalfWallBreakMove::finished( const PlayerAgent * agent )
{
    ...
    if ( ++M_count >= 13 ) // 2014-07-18
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) no remained step" );
        return true;
    }
    ...
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) time mismatch" );
        return true;
    }
    ...
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) hear pass message" );
        return true;
    }
    ...
    // {
    //     dlog.addText( Logger::TEAM,
    //                   __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) too over offside line" );
    //     return true;
    // }
    ...
    if ( wm.ball().pos().x > ServerParam::i().theirPenaltyAreaLineX() )
    {
        ...
        {
            dlog.addText( Logger::TEAM,
                          __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) situation is not attack" );
            return true;
        }
    }
    ...
    if ( wm.self().pos().x > wm.offsideLineX() )
    {
        ...
        {
            dlog.addText( Logger::TEAM,
                          __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) over offside line count" );
            return true;
        }
    }
    ...
    if ( opp_min < mate_min )
    {
        ++M_their_ball_count;
        dlog.addText( Logger::TEAM,
                      __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) their ball count = %d",
                      M_their_ball_count );
    ...
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) my ball" );
        return true;
    }
    ...
    // {
    //     dlog.addText( Logger::TEAM,
    //                   __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) ball holder changed." );
    //     return true;
    // }
    /*
    ...
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) home pos x >> target x." );
        return true;
    }
    */
    ...
    if ( my_pos.dist2( M_target_point ) < 1.5 )
    {
        ...
        {
            dlog.addText( Logger::TEAM,
                          __FILE__":(IntentionDefensiveHalfWallBreakMove::finished) exist marker." );
            return true;
        }
        */
        return true;
 ...
 */
bool
IntentionDefensiveHalfWallBreakMove::execute( PlayerAgent * agent )
{
    ...
    agent->debugClient().addMessage( "DHMove:Intention%d", M_count );
 ...
 */
bool
Bhv_DefensiveHalfWallBreakMove::execute( PlayerAgent * agent )
{
    dlog.addText( Logger::TEAM,
                  __FILE__": Bhv_DefensiveHalfWallBreakMove" );
 ...
 */
bool
Bhv_DefensiveHalfWallBreakMove::doCornerMove( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
         ...
         && wm.ball().pos().x < wm.offsideLineX() )
    {
        if( receive_pos.absY() > 20.0 )
        {
            ...
            if ( wm.self().pos().x > 38.0 )
            {
                target_point.x = wm.ball().pos().x - 4.0;
            }
            if ( position_type == Position_Right )
            {
                if ( wm.ball().pos().y > 0 ) target_point.y = 18.0;
                else target_point.y = -10.0;
            }
            else if ( position_type == Position_Left )
            {
                if ( wm.ball().pos().y > 0 ) target_point.y = 10.0;
                else target_point.y = -18.0;
            }
        }
        else if( receive_pos.absY() < 20.0 )
        {
            ...
            target_point.x = 43.0;
             if ( position_type == Position_Right )
            {
                if ( wm.ball().pos().y > 0 ) target_point.y = 8.0;
                else target_point.y = -2.0;
            }
            else if ( position_type == Position_Left )
            {
                if ( wm.ball().pos().y > 0 ) target_point.y = 2.0;
                else target_point.y = -8.0;
            }
        }
        ...
        if ( wm.ball().pos().x < 36.0
             && wm.self().pos().x < 36.0 )
        {
            agent->setIntention( new IntentionDefensiveHalfWallBreakMove( target_point, wm.time(), ball_holder_unum ) );
        }
    ...
    dlog.addText( Logger::ROLE,
                  ...
                  dash_power,
                  dist_thr );
    agent->debugClient().addMessage( "DH:WallBreakMove:%.0f", dash_power );
...
double
Bhv_DefensiveHalfWallBreakMove::evaluateTargetPoint( const rcsc::WorldModel & wm,
 ...
 */
bool
Bhv_DefensiveHalfWallBreakMove::isMoveSituation( PlayerAgent * agent  )
{
    dlog.addText( Logger::TEAM,
                  __FILE__": Bhv_DefensiveHalfWallBreakMove(isMoveSituation)" );
    ...
    else if ( ball_pos.x < 30.0 )
    {
        ...
        for ( double i = l_y; i > r_y; i -= 2.0 )
        {
            ...
            if ( isIntercept )
            {
                //std::cout << wm.time().cycle() << " (isMoveSituation) situation is side attack " << std::endl;
 ...
 */
bool
Bhv_DefensiveHalfWallBreakMove::checkOpponent( const WorldModel & wm,
