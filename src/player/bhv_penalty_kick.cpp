...
{
    dlog.addText( Logger::TEAM, __FILE__": (execute)" );
 ...
 */
bool
Bhv_PenaltyKick::doFielderWait( PlayerAgent * agent )
{
#if 1
...
Bhv_PenaltyKick::doKickerSetup( PlayerAgent * agent )
{
    ...
    if ( ! Body_GoToPoint( ball_pos, 0.5, ServerParam::i().maxDashPower() ).execute( agent ) )
    {
 ...
 */
bool
Bhv_PenaltyKick::doGetBall( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
...
{
    dlog.addText( Logger::TEAM, __FILE__" (doShoot)" );

    if ( Bhv_Shoot().execute( agent ) )
    {
        dlog.addText( Logger::TEAM,
                      __FILE__" (doShoot) normal shoot" );
        return true;
    }
    ...
    // {
    //     dlog.addText( Logger::TEAM,
    //                   __FILE__" (doShoot) normal shoot behavior." );
    //     return true;
    // }
    ...
    if ( opp_goalie
         && opp_goalie->pos().x - wm.self().pos().x > 4.5
         && wm.self().pos().x < 23.0
         && wm.ball().pos().dist( opp_goalie->pos() ) > 4.0 ) // MAGIC NUMBER
    {
        dlog.addText( Logger::TEAM,
                      __FILE__" (doShoot) goalie's position (%.1f, %.1f).",
                      opp_goalie->pos().x, opp_goalie->pos().y );
        dlog.addText( Logger::TEAM,
                      __FILE__" (doShoot) goalie is not close enough. should dribble more." );
    ...
    {
        if ( 36.0 < wm.self().pos().x
             && getShootTarget( agent, &shot_point, &shot_speed ) )
        {
            dlog.addText( Logger::TEAM,
...
Bhv_PenaltyKick::getShootTarget( const PlayerAgent * agent,
                                 ...
                                 double * first_speed )
{
    dlog.addText( Logger::TEAM, __FILE__" (getShootTarget)" );
    ...
    if ( 1 <= unum_opponent_goalie
         && unum_opponent_goalie <= 11
         && player_type_opponent_goalie )
    {
        goalie_max_speed = player_type_opponent_goalie->realSpeedMax();

        goalie_dist_buf
            = goalie_max_speed * std::min( 5, opp_goalie->posCount() )
            + player_type_opponent_goalie->maxCatchableDist()
            + 0.2;
    }
    ...
    for ( int i = 0; i <= SHOOT_RESOLUTION; ++i )
    {
        ...
        const Vector2D target( SP.pitchHalfLength(), shoot_range - i * ( (2.0 * shoot_range)  / SHOOT_RESOLUTION) ); // MAGIC NUMBER
...
Bhv_PenaltyKick::doDribble( PlayerAgent * agent )
{
    ...
    //
    /*
    if ( Bhv_SelfPassPenaltyKick().execute( agent ) )
    {
        return true;
    }
    */
    ...
    const double base_target_abs_y = ServerParam::i().penaltyAreaHalfWidth() - 2.0;
    ...
    */
        if ( goalie_abs_x > my_abs_x )
        {
            ...
            if ( opp_goalie
                 &&
                 ( ( drib_target.y < opp_goalie->pos().y
                     && opp_goalie->pos().y < agent->world().self().pos().y )
                   || ( agent->world().self().pos().y < opp_goalie->pos().y
                        && opp_goalie->pos().y < drib_target.y ) ) )
            {
                drib_target.x = goalie_abs_x + 5.0;
            }
            else
            {
                if( goalie_dist < 5.0 )
                {
                    drib_target.x = goalie_abs_x - 2.0;
                }
                else
                {
            ...
            drib_dashes = min_max( 1, drib_dashes, 5 ); // 6 MAGIC NUMBER
            dlog.addText( Logger::TEAM,
                          __FILE__":%d dribble. target=(%.1f, %.1f) dashes=%d",
                          __LINE__, drib_target.x, drib_target.y, drib_dashes );
        }
        else
        {
            drib_target = goal_c;
    ...
    if ( opp_goalie
         && wm.self().pos().x < opp_goalie->pos().x
         && goalie_dist < 5.0 )
    {
        ...
        {
        */
        static int s_going_toward_plus = 20 * ( wm.self().pos().y > 0.0
                                                ? - 1.0
                                                : + 1.0 );
        static int s_decrement = wm.self().pos().y > 0.0
            ? + 1
            : - 1;
            ...
            else
            {
                ...
                adjustment2.rotate( s_going_toward_plus > 0
                                    ? + 90.0
                                    : - 90.0 );
            ...
            dlog.addText( Logger::TEAM,
                          ...
                          __LINE__, drib_target.x, drib_target.y, s_going_toward_plus );
            /*
        }
            */
        dlog.addText( Logger::TEAM,
                      __FILE__":%d dribble. goalie near. dashes=%d",
                      __LINE__, drib_dashes );
    }
    ...
    if ( Strategy::i().opponentType() == Strategy::Type_Gliders )
    {
        if ( opp_goalie )
        {
            ...
            if ( wm.self().pos().x > penalty_abs_x )
            {
                drib_target.y = 0.0;
                drib_target.x = penalty_abs_x - 2.0;
            }

            if ( opp_goalie->pos().dist( wm.ball().pos() ) < 3.0 )
            {
                drib_target.y -= 10.0;
            }
        }
    ...
    else if ( Strategy::i().opponentType() == Strategy::Type_MarliK )
    {
        if ( opp_goalie )
        {
            drib_target.x = penalty_abs_x + 3.0;
            drib_target.y = 18.0;

            if ( opp_goalie->pos().absX() - wm.ball().pos().absX() < 12.0 )
            {
                drib_power *= 0.2;
            }

            if ( opp_goalie->pos().absX() - wm.self().pos().absX() < 5.2 )
            {
                drib_power = ServerParam::i().maxDashPower();
                drib_target.x =  penalty_abs_x;
                drib_target.y = -SP.penaltyAreaHalfWidth() - 10.0;
            }

            if ( penalty_abs_x - wm.self().pos().x < 1.5 )
            {
                drib_target.x = penalty_abs_x + 3.0;
                drib_target.y = SP.penaltyAreaHalfWidth() + 10.0;
            }
    ...
    else if ( Strategy::i().opponentType() == Strategy::Type_WrightEagle )
    {
        if ( opp_goalie )
        {
            drib_target.x = wm.self().pos().x;
                ...
                || wm.self().pos().x > 12.0 )
            {
                ...
                drib_target.x = wm.self().pos().x + 3.0;
            }
    ...
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":%d will try to look at opp goalie", __LINE__ );
        agent->setNeckAction( new Neck_TurnToPoint( opp_goalie->pos() ) );
    }
    else
    {
...
*/
bool
Bhv_PenaltyKick::doDribble2013( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    ...
    {
        dlog.addText( Logger::TEAM,
                      __FILE__"(doDribble2013) chase the ball" );
    ...
    dlog.addText( Logger::TEAM,
                  __FILE__"(doDribble2013) kick the ball" );

    if ( Bhv_ChainAction().execute( agent ) )
    {
        dlog.addText( Logger::TEAM,
                      __FILE__"(doDribble2013) done chain action" );
        agent->setNeckAction( new Neck_TurnToGoalieOrScan( -1 ) );
        return true;
    }
...
Bhv_PenaltyKick::doGoalie( PlayerAgent* agent )
{
    ...
    ///////////////////////////////////////////////
    if ( Bhv_SaviorPenaltyKick().execute( agent ) )
    {
        return true;
    }
