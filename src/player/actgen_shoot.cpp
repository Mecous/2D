                        ...
                        const std::vector< ActionStatePair > & ) const
{
    ...
    state.getPlayers( opponents, new OpponentOrUnknownPlayerPredicate( wm ) );

    if ( ! ShootSimulator::can_shoot_from( holder.unum() == wm.self().unum(),
                                           holder.pos(),
                                           opponents,
    ...
    CooperativeAction::Ptr action( new ActShoot( holder.unum(),
                                                 ServerParam::i().theirTeamGoalPos(),
