...
Neck_DefaultInterceptNeck::execute( PlayerAgent * agent )
{
    ...
    else
    {
        ...
        if ( opponent_step <= 1 )
        {
            ...
            const AngleDeg max_move_angle = AngleDeg::asin_deg( ServerParam::i().ballSpeedMax()
                                                                / wm.ball().distFromSelf() );
            if ( next_view_width - 5.0 < max_move_angle.abs() )
            {
                ball_count_thr = -1;
            }
        }
        ...
        dlog.addText( Logger::INTERCEPT,
                      __FILE__": default ball_or_scan. ball_count_thr=%d",
                      ball_count_thr );

        Neck_TurnToBallOrScan( ball_count_thr ).execute( agent );
    }
