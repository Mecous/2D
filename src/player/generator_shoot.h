...
#ifndef GENERATOR_SHOOT_H
#define GENERATOR_SHOOT_H
...
private:
    ...
    // not used
    GeneratorShoot( const GeneratorShoot & );
    const GeneratorShoot & operator=( const GeneratorShoot & );
public:
    ...
    /*!
      \brief get the best shoot course.
      \return const pointer to the course object. if no result, NULL is returned.
     */
    const Course * getBestCourse( const rcsc::WorldModel & wm );
