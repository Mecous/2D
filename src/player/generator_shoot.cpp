 ...
 */
GeneratorShoot &
GeneratorShoot::instance()
{
    static GeneratorShoot s_instance;
    return s_instance;
}
 ...
 */
const GeneratorShoot::Course *
GeneratorShoot::getBestCourse( const WorldModel & wm )
{
    const Container & cont = this->courses( wm );

    if ( cont.empty() )
    {
        dlog.addText( Logger::SHOOT,
                      __FILE__": empty shoot course" );
        return static_cast< const Course * >( 0 );
    }

    Container::const_iterator best_shoot
        = std::min_element( cont.begin(),
                            cont.end(),
                            GeneratorShoot::ScoreCmp() );
 ...
 */
void
GeneratorShoot::generate( const WorldModel & wm )
{
 ...
 */
void
GeneratorShoot::createShoot( const WorldModel & wm,
                             const Vector2D & target_point )
{
    ...
    if ( goalie
         && 5 < goalie->posCount()
         && goalie->posCount() < 30
         && wm.dirCount( ball_move_angle ) >= 6 )
    {
#ifdef DEBUG_PRINT
        dlog.addText( Logger::SHOOT,
                      "%d: __ xxx goalie_count=%d, dir_count=%d, low accuracy",
                      M_total_count,
                      goalie->posCount(),
                      wm.dirCount( ball_move_angle ) );
#endif
        return;
    }
#endif
    ...
    // check goal post
    {
        const Segment2D ball_move_line( M_first_ball_pos, target_point );
        Vector2D post_l( SP.pitchHalfLength() - SP.goalPostRadius(), -SP.goalHalfWidth() + SP.goalPostRadius() );
        Vector2D post_r( SP.pitchHalfLength() - SP.goalPostRadius(), +SP.goalHalfWidth() - SP.goalPostRadius() );
        if ( ball_move_line.dist( post_l ) < SP.goalPostRadius() + SP.ballSize() + 0.05
             || ball_move_line.dist( post_r ) < SP.goalPostRadius() + SP.ballSize() + 0.05 )
        {
#ifdef DEBUG_PRINT
            dlog.addText( Logger::SHOOT,
                          "%d: __ xxx maybe collide with goal post", M_total_count );
#endif
            return;
        }
    }
                             ...
                             const double & ball_move_dist )
{
    ...
    dlog.addText( Logger::SHOOT,
                  "%d: ----- "
                  //"target=(%.2f %.2f) "
                  "speed=%.3f angle=%.1f ball_reach_step=%d",
                  M_total_count,
                  //target_point.x, target_point.y,
                  first_ball_speed,
                  ball_move_angle.degree(),
 ...
 */
bool
GeneratorShoot::maybeGoalieCatch( const PlayerObject * goalie,
                                  Course & course )
{
 ...
 */
bool
GeneratorShoot::opponentCanReach( const PlayerObject * opponent,
                                  Course & course )
{
    const ServerParam & SP = ServerParam::i();
 ...
 */
void
GeneratorShoot::evaluateCourses( const WorldModel & wm )
{
    const ServerParam & SP = ServerParam::i();
    const AbstractPlayerObject * goalie = wm.getTheirGoalie();
