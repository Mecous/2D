...
Neck_OffensiveInterceptNeck::execute( PlayerAgent * agent )
{
    ...
    const InterceptInfo intercept = Body_Intercept::get_best_intercept( wm, true );
    const int self_min = intercept.reachCycle(); //wm.interceptTable()->selfReachCycle();
    ...
    dlog.addText( Logger::TEAM,
                  __FILE__": self_step = %d, ball_angle_from_body=%.1f angle_diff = %.1f",
                  self_min, ball_angle.degree(), angle_diff );
    ...
    if ( ! view
         && ! ball_in_visible
         && self_min >= 4
         && angle_diff > ViewWidth::width( rcsc::ViewWidth::NORMAL ) * 0.5 - 10.0 )
    {
        view = new View_Wide();
        dlog.addText( Logger::TEAM,
    ...
    if ( ! view
         && ! ball_in_visible
         && self_min >= 3
         && angle_diff > ViewWidth::width( rcsc::ViewWidth::NARROW ) * 0.5 - 10.0 )
    {
        view = new View_Normal();
        dlog.addText( Logger::TEAM,
    ...
    //else if ( ball_in_visible )
              ...
              && trap_ball_speed < wm.self().playerType().kickableArea() * 1.2 )
    {
        agent->debugClient().addMessage( "OffenseNeck:Teammate" );
        //agent->debugClient().addMessage( "OffenseNeck:Scan" );
        dlog.addText( Logger::TEAM,
                      __FILE__": default neck: scan field" );
        Neck_DefaultInterceptNeck( view,
                                   new Neck_TurnToLowConfTeammate() ).execute( agent );
        // Neck_DefaultInterceptNeck( view,
        //                            new Neck_ScanField() ).execute( agent );
    }
    // TODO:
...
Neck_OffensiveInterceptNeck::doTurnNeckToShootPoint( PlayerAgent * agent )
{
    ...
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":(doTurNeckToShootPoint) no 1 step trap" );
        return false;
    }

    const GeneratorShoot::Course * best_shoot = GeneratorShoot::instance().getBestCourse( wm );

    if ( ! best_shoot )
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":(doTurNeckToShootPoint) no best shoot?" );
        return false;
    }

    dlog.addText( Logger::TEAM,
                  __FILE__":(doTurNeckToShootPoint) shoot target (%.2f %.2f)",
                  best_shoot->target_point_.x, best_shoot->target_point_.y );
    ...
    {
        dlog.addText( Logger::TEAM,
                      __FILE__":(doTurNeckToShootPoint) cannot look the shoot point(%.2f %.2f)",
    ...
    if ( wm.seeTime() == wm.time() )
    {
        ...
        {
            dlog.addText( Logger::TEAM,
                          __FILE__":(doTurNeckToShootPoint) already seen. width=%.1f, diff=%.1f. shoot point(%.2f %.2f)",
                          current_width,
                          angle_diff,
                          best_shoot->target_point_.x,
                          best_shoot->target_point_.y );
    ...
    dlog.addText( Logger::TEAM,
                  __FILE__":(doTurNeckToShootPoint) turn_neck to the shoot point(%.2f %.2f)",
                  best_shoot->target_point_.x,
                  best_shoot->target_point_.y );
